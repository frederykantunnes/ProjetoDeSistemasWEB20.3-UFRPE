$(function() {
    $('input[type=radio]').change(function(){
        ...
        xmlAjax.setRequestHeader("X-CSRF-TOKEN", document.head.querySelector("[name=csrf-token]").content );
        xmlAjax.onload = function (e) {
          if (xmlAjax.readyState === 4) {
            if (xmlAjax.status === 200) {
                ...
                $("#span"+poll_id).removeClass("d-none");
                ...
                for ([key, value] of Object.entries(data.Options)) {
                  ...
                  $("#progressdiv"+key).removeClass("d-none");
                  $("#progress"+key).attr("aria-valuenow", value.toFixed(2));
                  $("#progress"+key).attr("style", "width: "+ value.toFixed(2) + "%");
                  $("#progress"+key).text(value.toFixed(2) + "% " + $("#label"+key).html());
                }

                $("#progress"+option_id).removeClass("bg-info");
