...
@section('content')
    <div class="container">
        @foreach($polls as $poll)
            <div class="card">
                <div class="card-header bg-dark text-white" id="{{'question'.strval($poll->id)}}">
                <span class="float-left">{{$poll->question}}</span>
                ...
                @foreach($options_array[$poll->id] as $option)
                        <div class="form-check mb-3">
                            ...
                            <div class="progress-bar" id="{{'progress'.strval($option->id)}}" role="progressbar" style="width: 100%" aria-valuenow="100" aria-valuemin="0" aria-valuemax="100"></div>
                            </div>
                    </div>
                @endforeach
            </div>
